...
void GameController::updateCheckState() const {
  const PiecesConstants::PIECE_COLORS opponentColor = gameState.getInactiveColor();
  ...
  if (this->model->isUnderAttack(posOpponentKing)) {
    ...
    gameState.setCheck(opponentColor);
  }
}
...
void GameController::endGameCheck() {
  if (!this->model->isPlayerHavePossibleMove()) {
    const std::optional<PiecesConstants::PIECE_COLORS> checkColor =
      gameState.getCheckColor();

    if (checkColor.has_value())
      gameState.setGameStatus(GameStateConstants::GAME_STATUS::CHECKMATE);
    else gameState.setGameStatus(GameStateConstants::GAME_STATUS::STALEMATE);
...
void GameController::clearCheckAndLastMove() const {
  if (gameState.getCheckColor().has_value()) {
    this->view->clearHighlights({
      .isCheck = true,
      .isLastMove = true
    });
    gameState.clearCheck();
  } else {
...
QString GameController::getFEN() const {
  ...
  const char activeColorInFEN = ConstantsFEN::colorInFEN[
    gameState.getActiveColor()];
  const std::string castling = this->model->getCastlingForFEN();
  const std::string enPassant = gameState.getStringEnPassant();
  const size_t halfMoveClock = this->model->getHalfMoveClock();
  const size_t fullMoveNumber = gameState.getFullMoveNumber();
